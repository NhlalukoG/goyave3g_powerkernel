...
void URLRequestJob::NotifyHeadersComplete() {
  ...
  if (IsRedirectResponse(&new_location, &http_status_code)) {
    ...
    if (auth_info) {
	negotiateAuthState = isNegotiatePresent(request_->response_headers());              //samsung sso integration
...
// samsung sso integration
int URLRequestJob::negotiateAuthState = 0;

int URLRequestJob::getNegotiateAuthState() {
  return URLRequestJob::negotiateAuthState;
}

int URLRequestJob::isNegotiatePresent(HttpResponseHeaders* headers) {
  int iState = 0;
  std::string scheme;
...
if (headers) {
  ...
  if (http_status_code == 401) {
	headers->GetNormalizedHeader("WWW-Authenticate",&scheme);
  } else if (http_status_code == 407) {
	headers->GetNormalizedHeader("Proxy-Authenticate",&scheme);
  } else {
    LOG(INFO) << "URLRequestJob:isNegotiatePresent can't find auth header";
    return -1;
  }
  std::string lower_schem = StringToLowerASCII(scheme);
  std::size_t foundNegotiate = lower_schem.find("negotiate");
  std::size_t foundNTLM = lower_schem.find("ntlm");
  std::size_t foundDigest = lower_schem.find("digest");
  std::size_t foundBasic = lower_schem.find("basic");
  if (foundNegotiate != std::string::npos) {
    iState |= 0x1;
  }

  if (foundNTLM != std::string::npos || foundDigest != std::string::npos || foundBasic != std::string::npos) {
    iState |= 0x2;
  }
  return iState;
  }
