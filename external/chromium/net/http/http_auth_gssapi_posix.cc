...
std::string GSSAPISharedLibrary::browserAppDataPath = "";

void GSSAPISharedLibrary::setBrowserAppDataPath(std::string path) {
    GSSAPISharedLibrary::browserAppDataPath = path;
}

bool GSSAPISharedLibrary::Init() {
  int ret_conf;
  int ret_cc;
  std::string temp1 = GSSAPISharedLibrary::browserAppDataPath;
  std::string temp2 = GSSAPISharedLibrary::browserAppDataPath;
  if (!temp1.empty()) {
    temp1.append("/kerberos/krb5.conf"); 
	temp2.append("/kerberos/ccache/krb5cc"); 
	ret_conf = setenv("KRB5_CONFIG", temp1.c_str(), 1); 
    ret_cc = setenv("KRB5CCNAME", temp2.c_str(), 1);
  }
  if (!initialized_)
    InitImpl();
  return initialized_;
...
base::NativeLibrary GSSAPISharedLibrary::LoadSharedLibrary() {
  ...
  if (!gssapi_library_name_.empty()) {
    ...
    static const char* kDefaultLibraryNames[] = {
	  ...
	  "libkerberos.so"        // Lib Kerberos for android
#endif
    };
...
OM_uint32 GSSAPISharedLibrary::inquire_cred( 
      OM_uint32* minor_status, 
  ...
  DCHECK(initialized_); 
  return inquire_cred_(minor_status, 
		               cred_handle, 
		               name, 
		               lifetime, 
		               cred_usage, 
...
bool HttpAuthGSSAPI::TGTExpired = false;

bool HttpAuthGSSAPI::isTGTExpired() {
  return HttpAuthGSSAPI::TGTExpired;
}
int HttpAuthGSSAPI::GetNextSecurityToken(const std::wstring& spn,
  ...
  if (rv != OK) {
    LOG(ERROR) << "Problem initializing context. \n"
	  ...
	  OM_uint32 vmajor_status = 0; 
      OM_uint32 vminor_status = 0; 
      gss_name_t vname = GSS_C_NO_NAME; 
      OM_uint32 vlifetime = 0; 
      gss_cred_usage_t vcred_usage = 0; 
      gss_OID_set vmech_type = GSS_C_NO_OID_SET; 
      vmajor_status = library_->inquire_cred(&vminor_status, 
                         			         GSS_C_NO_CREDENTIAL, 
                          			         &vname, 
                         			         &vlifetime, 
                          			         &vcred_usage, 
                          			         &vmech_type); 
      if (vmajor_status != GSS_S_COMPLETE)  
   	    TGTExpired = true; 
  }
  return rv;
}
