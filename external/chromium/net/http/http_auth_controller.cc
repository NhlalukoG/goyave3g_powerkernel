...
void HttpAuthController::PopulateKerberosAuthChallenge() {
  DCHECK(CalledOnValidThread());
  if (kerberos_auth_info_ != NULL) {
  	auth_info_ = kerberos_auth_info_;
  }
}
    ...
    switch (result) {
      ...
      case HttpAuth::AUTHORIZATION_RESULT_REJECT: {
        ...
        HistogramAuthEvent(handler_.get(), AUTH_EVENT_REJECT);
		 if (handler_->auth_scheme() == 3) { 
		  kerberos_auth_info_ = new AuthChallengeInfo;
		  kerberos_auth_info_->is_proxy = (target_ == HttpAuth::AUTH_PROXY);
		  kerberos_auth_info_->host_and_port = ASCIIToWide(GetHostAndPort(auth_origin_));
		  kerberos_auth_info_->scheme = ASCIIToWide(HttpAuth::SchemeToString(handler_->auth_scheme()));
	  	  kerberos_auth_info_->realm = ASCIIToWide(handler_->realm());
	    }
        InvalidateCurrentHandler(INVALIDATE_HANDLER_AND_CACHED_CREDENTIALS);
		}
        break;
