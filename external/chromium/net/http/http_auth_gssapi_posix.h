...
class GSSAPILibrary {
 public:
  ...
  virtual OM_uint32 inquire_context(
      ...
      int* open) = 0;
 virtual OM_uint32 inquire_cred( 
      OM_uint32* minor_status, 
...
virtual OM_uint32 inquire_cred( 
  ...
  static std::string browserAppDataPath;
  static void setBrowserAppDataPath(std::string path);
 private:
  ...
  gss_inquire_cred_type inquire_cred_;
};
...
class HttpAuthGSSAPI {
 public:
  ...
  void Delegate();
static bool TGTExpired;
  static bool isTGTExpired();
 private:
