...
#include <edmnativehelper/IEDMNativeHelperService.h>
#include <edmnativehelper/EDMNativeHelper.h>
#endif
//WTL_EDM_END - 5.0
...
#ifdef SEC_PRODUCT_FEATURE_MDM_SECEDM_ENABLE     
// Size of header msg
#define AUDITLOG_MSG_SIZE 34
// EdmNativeHelper extern function
extern "C" void c_auditLog(int severityGrade, int moduleGroup, bool outcome, int uid,
    const char* swComponent, const char* logMessage) {
     android::EDMNativeHelper::nativeLogger(severityGrade, moduleGroup, outcome, uid,
    ...
    for (std::vector<FormField>::iterator iter = result.fields.begin();
      ...
      if ((*iter) == field) {
        ...
        if (profile) {
          ...
          //WTL_EDM_START - 5.0
          #ifdef SEC_PRODUCT_FEATURE_MDM_SECEDM_ENABLE
          value = profile->GetInfo(field_type);
          profile_full_name = profile->GetInfo(NAME_FULL);
          // Total size of msg
          total_length = value.length() + profile_full_name.length() + AUDITLOG_MSG_SIZE;
          // If msg exists
          if (total_length != 0){
              // Allocated memory with msg size
              audit_result = (char*)malloc(total_length*sizeof(char));
              // Verify if malloc works
              if(audit_result){
                  sprintf(audit_result, "Auto-completing data for field %s %s",UTF16ToUTF8(value).c_str(),UTF16ToUTF8(profile_full_name).c_str());
                  // Send autofill msg to auditlog
                  c_auditLog(1,1,1,1,"RequestFormData",audit_result);
                  free(audit_result);
              }
          }
          #endif
